package frc.robot;

public class RobotMap {

	// Ports not on robot
	// Joysticks
	public static final int JOYSTICK_DRIVE_LEFT = 0;
	public static final int JOYSTICK_DRIVE_RIGHT = 1;
	public static final int JOYSTICK_MANIPULATOR = 2;

	public static final int DRIVE_CONTROLLER = 4;

	//Joystick Buttons

	// Robot Ports
	// Analog Ports

	// Drive Train Motors
	public static final int NEO_FRONT_LEfT = 0;
	public static final int NEO_FRONT_RIGHT = 1;
	public static final int NEO_BACK_RIGHT = 2;
	public static final int NEO_BACK_LEFT = 3;

	public static final int MOTOR_FRONT_LEFT = 0;
	public static final int MOTOR_FRONT_RIGHT = 1;
	public static final int MOTOR_BACK_LEFT = 2;
	public static final int MOTOR_BACK_RIGHT = 3;

	// Drive Train Encoders
	public static final int DRIVE_TRAIN_LEFT_ENCODER_A = 0;
	public static final int DRIVE_TRAIN_LEFT_ENCODER_B = 1;
	public static final int DRIVE_TRAIN_RIGHT_ENCODER_A = 2;
	public static final int DRIVE_TRAIN_RIGHT_ENCODER_B = 3;

}